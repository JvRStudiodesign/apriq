...
// ── 1. Create single consolidated send-email.js ───────────────────────────────
const sendEmail = `
...
fs.writeFileSync('api/send-email.js', sendEmail, 'utf8');
console.log('✓ send-email.js created');

// ── 2. Delete all old individual email endpoints ──────────────────────────────
const toDelete = [
  'api/send-welcome.js','api/send-trial-warning.js','api/send-trial-expired.js',
  'api/send-payment-confirmed.js','api/send-payment-failed.js','api/send-cancelled.js',
  'api/send-contact.js','api/send-feedback.js',
  'api/send-new-user.js','api/send-new-client.js','api/send-new-waitlist.js',
];
toDelete.forEach(f => { if (fs.existsSync(f)) { fs.unlinkSync(f); console.log('✓ deleted', f); } });

// ── 3. Update all callers ──────────────────────────────────────────────────────
const replacements = [
  ['/api/send-welcome', '/api/send-email', 'type', 'welcome'],
  ['/api/send-trial-warning', '/api/send-email', 'type', 'trial_warning'],
  ['/api/send-trial-expired', '/api/send-email', 'type', 'trial_expired'],
  ['/api/send-payment-confirmed', '/api/send-email', 'type', 'payment_confirmed'],
  ['/api/send-payment-failed', '/api/send-email', 'type', 'payment_failed'],
  ['/api/send-cancelled', '/api/send-email', 'type', 'cancelled'],
  ['/api/send-contact', '/api/send-email', 'type', 'contact'],
  ['/api/send-feedback', '/api/send-email', 'type', 'feedback'],
  ['/api/send-new-user', '/api/send-email', 'type', 'new_user'],
  ['/api/send-new-client', '/api/send-email', 'type', 'new_client'],
  ['/api/send-new-waitlist', '/api/send-email', 'type', 'new_waitlist'],
];
...
filesToUpdate.forEach(path => {
  ...
  let changed = false;
  replacements.forEach(([oldUrl, newUrl, typeKey, typeVal]) => {
    if (content.includes(oldUrl)) {
      // Replace the URL and inject type into the body
      content = content.split(oldUrl).join(newUrl);
      // Add type to JSON body — find the body: JSON.stringify({ pattern
      content = content.replace(
        new RegExp(`(fetch\\('${newUrl.replace('/','\\/')}[^)]*\\{\\s*method:'POST'[^}]*body:\\s*JSON\\.stringify\\()\\{`, 'g'),
        (m) => m.includes('"type"') || m.includes("'type'") ? m : m.replace(/\{$/, `{ type:'${typeVal}', `)
      );
      changed = true;
    }
  });
  if (changed) { fs.writeFileSync(path, content, 'utf8'); console.log('✓ updated', path); }
});

// Also update payfast-itn.js which uses different pattern
let itn = fs.readFileSync('api/payfast-itn.js', 'utf8');
itn = itn.replace(`'/api/send-payment-confirmed'`, `'/api/send-email'`);
itn = itn.replace(`'/api/send-cancelled'`, `'/api/send-email'`);
itn = itn.replace(`'/api/send-payment-failed'`, `'/api/send-email'`);
// Add type to each body
itn = itn.replace(
...
console.log('✓ payfast-itn.js updated');

console.log('\nAPI count now:', fs.readdirSync('api').length, 'files');
