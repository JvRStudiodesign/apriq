        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '1.5rem', width: '100%', maxWidth: '400px', margin: '1rem' }}>
            ...
            {fbSent ? <p style={{ color: '#27ae60', textAlign: 'center', padding: '1rem 0' }}>Thanks — noted.</p> : (
              ...
              fetch('/api/send-email', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ email: user?.email, topic: fbTopic, message: fbText }) }).then(r=>r.json()).then(d=>console.log('feedback email:',d)).catch(e=>console.error('feedback email failed:',e));
              if (user && user.email) fetch('/api/send-email', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:'feedback_confirm', to: user.email }) }).catch(()=>{}); setFbSent(true); setTimeout(() => { setFeedback(false); setFbSent(false); setFbText(''); setFbTopic('UI'); }, 2000); }}>
