...
const numIn = { width: '100%', padding: '0.6rem 0.875rem', border: '1.5px solid #e5e5e3', borderRadius: '10px', fontSize: '0.875rem', background: '#fff', color: '#1a1a18', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' };
const divider = { borderTop: '1px solid #f0f0ee', margin: '1.25rem 0' };
const PRO_BADGE = <span style={{ marginLeft: '5px', fontSize: '0.6rem', background: '#f0f0ee', color: '#aaa', padding: '1px 5px', borderRadius: '6px', verticalAlign: 'middle', fontWeight: '600', letterSpacing: '0.03em' }}>PRO</span>;
...
function BtnGroup({ label, value, onChange, options, locked, cols }) {
  return (
  ...
  return (
    <div style={{ marginBottom: '1.1rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', border: '1.5px solid #e5e5e3', borderRadius: '10px', overflow: 'hidden' }}>
        <input type="number" value={value} onChange={e => onChange(parseFloat(e.target.value) || 0)} style={{ flex: 1, padding: '0.6rem 0.875rem', border: 'none', outline: 'none', fontSize: '0.875rem', fontFamily: 'inherit', color: '#1a1a18', background: '#fff' }} />
...
const projectOpts    = Object.entries(PROJECT_TYPE).map(([k, v]) => ({ value: k, label: v.label }));
const renovOpts      = Object.entries(RENOVATION_COMPLEXITY).map(([k, v]) => ({ value: k, label: v.label }));
const complexityOpts = [{ value: 'Low Complexity', label: 'Low' }, { value: 'Medium Complexity', label: 'Medium' }, { value: 'High Complexity', label: 'High' }];
const siteOpts       = [{ value: 'Urban Setting', label: 'Urban' }, { value: 'Suburban Setting', label: 'Suburban' }, { value: 'Peri-Urban Setting', label: 'Peri-Urban' }, { value: 'Rural Setting', label: 'Rural' }, { value: 'Exurban Setting', label: 'Exurban' }, { value: 'Specialized/Natural Setting', label: 'Specialized' }];
const slopeOpts      = [{ value: 'Flat Land (0-5%)', label: 'Flat' }, { value: 'Moderately Sloped Land (5-15%)', label: 'Moderate' }, { value: 'Steep / Hilly Land (15%+)', label: 'Steep' }, { value: 'Irregular / Constrained Land', label: 'Irregular' }];
...
export default function Calculator() {
  ...
  // Live calc for Pro
  useEffect(() => {
    if (isPro) setResult(calculate(inputs));
  }, [inputs, isPro]);
  ...
  function updCat1(catKey) {
    const cat = CATEGORIES.find(c => c.key === catKey);
    setInputs(p => ({ ...p, use1Category: catKey, use1Subtype: cat?.subtypes[0]?.key || '' }));
  }
  function updCat2(catKey) {
    const cat = CATEGORIES.find(c => c.key === catKey);
    setInputs(p => ({ ...p, use2Category: catKey, use2Subtype: cat?.subtypes[0]?.key || '' }));
  }
  function updCat3(catKey) {
    const cat = CATEGORIES.find(c => c.key === catKey);
    setInputs(p => ({ ...p, use3Category: catKey, use3Subtype: cat?.subtypes[0]?.key || '' }));
  }

  // Allocation sliders — auto-update dependent %
  ...
  function setAlloc2(pct) {
    ...
    setInputs(p => ({ ...p, use2Allocation: clamped, use3Allocation: numCats === 3 ? Math.round((max - clamped) * 100) / 100 : 0 }));
    setSaved(false);
  }
  ...
  function removeCategory(n) {
    if (n === 3) { setNumCats(2); setInputs(p => ({ ...p, use3Category: null, use3Subtype: null, use3Allocation: 0, use2Allocation: Math.round((1 - p.use1Allocation) * 100) / 100 })); }
    else if (n === 2) { setNumCats(1); setInputs(p => ({ ...p, use2Category: null, use2Subtype: null, use2Allocation: 0, use3Category: null, use3Subtype: null, use3Allocation: 0, use1Allocation: 1 })); }
  }
    ...
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #eeede8', padding: '0.875rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.875rem' }}>
          {trialOk && daysLeft <= 5 && (
            <span style={{ fontSize: '0.72rem', background: '#fff3cd', color: '#856404', padding: '2px 8px', borderRadius: '8px' }}>Trial — {daysLeft}d left</span>
          )}
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '1.5rem 1.25rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', alignItems: 'start' }}>
        ...
        <div>
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              ...
              {numCats < 3 && (
                <button onClick={addCategory}
                  style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '4px 12px', borderRadius: '9px', border: '1.5px solid #e5e5e3', background: '#fff', color: isPro ? '#1a1a18' : '#ccc', fontSize: '0.78rem', fontWeight: '500', cursor: 'pointer', fontFamily: 'inherit' }}>
            ...
            <select value={inputs.use1Category} onChange={e => updCat1(e.target.value)} style={{ ...sel, marginBottom: '0.85rem' }}>
            ...
            {numCats >= 2 && (
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                  ...
                  <button onClick={() => removeCategory(2)} style={{ background: 'none', border: 'none', color: '#ccc', cursor: 'pointer', fontSize: '1.1rem', lineHeight: 1, padding: '0 2px' }}>×</button>
                </div>
                <label style={lbl}>Category</label>
                <select value={inputs.use2Category || ''} onChange={e => updCat2(e.target.value)} style={{ ...sel, marginBottom: '0.85rem' }}>
                  {categoryOpts.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
                <label style={lbl}>Building type</label>
                ...
                {numCats === 2
                  ? <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.25rem 0', marginBottom: '0.25rem' }}><span style={lbl}>Allocation — Use 2</span><span style={{ fontSize: '0.875rem', fontWeight: '600', color: '#1a1a18' }}>{Math.round(inputs.use2Allocation * 100)}%</span></div>
            ...
            {numCats >= 3 && (
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                  ...
                  <button onClick={() => removeCategory(3)} style={{ background: 'none', border: 'none', color: '#ccc', cursor: 'pointer', fontSize: '1.1rem', lineHeight: 1, padding: '0 2px' }}>×</button>
                </div>
                <label style={lbl}>Category</label>
                <select value={inputs.use3Category || ''} onChange={e => updCat3(e.target.value)} style={{ ...sel, marginBottom: '0.85rem' }}>
                  {categoryOpts.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
                <label style={lbl}>Building type</label>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.25rem 0' }}><span style={lbl}>Allocation — Use 3</span><span style={{ fontSize: '0.875rem', fontWeight: '600', color: '#1a1a18' }}>{Math.round(inputs.use3Allocation * 100)}%</span></div>
              </>
            )}
            ...
            {inputs.projectTypeKey === 'Renovation' ? (
              <>
                ...
                <BtnGroup label="Renovation complexity" value={inputs.renovationComplexityKey} onChange={v => upd('renovationComplexityKey', v)} options={renovOpts} />
              </>
            ) : (
          ...
          <div style={card}>
            ...
            <BtnGroup label="Quality" value={inputs.qualityKey} onChange={v => upd('qualityKey', v)} options={qualityOpts} />
            <BtnGroup label="Site access" value={inputs.siteAccessKey} onChange={v => upd('siteAccessKey', v)} options={siteOpts} cols={3} />
            <BtnGroup label="Building complexity" value={inputs.complexityKey} onChange={v => upd('complexityKey', v)} options={complexityOpts} />
          </div>
          ...
          <div style={card}>
            ...
            {inputs.landProcurementType !== 'N/A' && (
              <>
                ...
                <BtnGroup label="Land type / slope" value={inputs.landSlopeKey} onChange={v => upd('landSlopeKey', v)} options={slopeOpts} cols={2} />
              </>
            )}
          </div>
          ...
          <div style={card}>
            ...
            <div style={{ opacity: isPro ? 1 : 0.4 }}>
              <label style={lbl}>Estimated start date{!isPro && PRO_BADGE}</label>
              <input type="date" value={inputs.estimatedStartDate || ''} onChange={e => isPro && upd('estimatedStartDate', e.target.value || null)} disabled={!isPro} style={{ ...numIn, cursor: isPro ? 'auto' : 'not-allowed' }} />
            </div>
          </div>

          {/* Calculate button for free users */}
          {!isPro && (
            <button onClick={handleCalc}
              style={{ width: '100%', padding: '0.875rem', background: '#1a1a18', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '0.9rem', fontWeight: '600', cursor: 'pointer', marginBottom: '1.5rem', fontFamily: 'inherit' }}>
              Calculate estimate
            </button>
          )}
        </div>
          ...
          {!result ? (
            <div style={{ ...card, textAlign: 'center', padding: '3rem 1.5rem' }}>
              <p style={{ color: '#ccc', fontSize: '0.875rem' }}>
                {isPro ? 'Adjust any input to see a live estimate.' : <>Fill in inputs and click <strong style={{ color: '#888' }}>Calculate estimate</strong></>}
              </p>
            </div>
          ) : (
            <>
              <div style={{ background: '#1a1a18', borderRadius: '14px', padding: '1.5rem', marginBottom: '1rem', color: '#fff' }}>
                <p style={{ fontSize: '0.68rem', color: '#555', marginBottom: '0.3rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total project cost</p>
                <p style={{ fontSize: '2rem', fontWeight: '700', letterSpacing: '-1px', lineHeight: 1.1 }}>{formatZAR(result.totalProjectCost)}</p>
                {result.escalatedTotal !== result.totalProjectCost && (
                  <p style={{ fontSize: '0.78rem', color: '#555', marginTop: '0.5rem' }}>Escalated ({result.monthsToStart} months): {formatZAR(result.escalatedTotal)}</p>
                )}
                {isPro && <p style={{ fontSize: '0.68rem', color: '#444', marginTop: '0.5rem' }}>Updates live as you adjust inputs</p>}
              </div>
              ...
              <div style={card}>
                ...
                {[
                  ...
                  { label: 'Construction (new)', value: result.baseConstructionCostNew },
                  { label: 'Construction (renovation)', value: result.baseConstructionCostRenovation },
                  ...
                  <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.45rem 0', borderBottom: '1px solid #f5f5f3', fontSize: '0.82rem' }}>
                    ...
                    <span style={{ fontWeight: '500', color: '#1a1a18' }}>{formatZAR(row.value)}</span>
                  </div>
                ))}
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '0.75rem', fontSize: '0.95rem', fontWeight: '700' }}>
              ...
              <div style={card}>
                ...
                <p style={{ fontSize: '1.5rem', fontWeight: '700', letterSpacing: '-0.5px' }}>{formatZAR(result.weightedBaseRate)}<span style={{ fontSize: '0.78rem', fontWeight: '400', color: '#bbb' }}> / m²</span></p>
      ...
      <button onClick={() => setFeedback(true)}
        style={{ position: 'fixed', bottom: '1.5rem', right: '1.5rem', background: '#1a1a18', color: '#fff', border: 'none', borderRadius: '20px', padding: '0.5rem 1rem', fontSize: '0.78rem', fontWeight: '500', cursor: 'pointer', boxShadow: '0 2px 12px rgba(0,0,0,0.15)', zIndex: 50, fontFamily: 'inherit' }}>
        Feedback
      </button>
