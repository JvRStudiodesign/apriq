...
export default async function handler(req, res) {
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [to],
      subject: `Payment confirmed — welcome to AprIQ Pro`,
...
<h2 style="font-size:1.25rem;font-weight:700;margin:0 0 0.75rem">You're on Pro.</h2>
<p style="color:#555;line-height:1.6;margin:0 0 1rem">Hi ${data.name || 'there'}, your payment of <strong>R ${data.amount || '149'}</strong> has been confirmed. Your AprIQ Pro subscription is now active.</p>
...
<p style="color:#aaa;font-size:0.8rem;margin-top:1.25rem">Manage your subscription at <a href="https://apriq.vercel.app/billing" style="color:#aaa">apriq.vercel.app/billing</a></p>
<div style="margin-top:2rem;padding-top:1rem;border-top:1px solid #eee;font-size:0.75rem;color:#aaa">AprIQ &mdash; apriq.co.za &mdash; apriq@apriq.co.za</div></div></body>`,
  ...
  if (!r.ok) { console.error('send-payment-confirmed error:', json); return res.status(500).json(json); }
